...
p_{infect\ DSF},\ p_{infect\ WT} &\sim Uniform(0, 1) \\
\ \\
...
source plant xf concentration in genotype DSF plants, is essentially `r round(res["b[2]","Mean"],3)`,
slightly lower than the genotype DSF constant term.  This implies that
for the same $distance$, genotype WT plants will have less xf
bacteria than genotype DSF plants; specifically, lower by
a factor of $e^{\beta_2-\beta_1}=$ `r round(exp(res["b[2]","Mean"] -
res["b[1]","Mean"]), 3)`.
...
vector xf concentration in genotype WT plants, is essentially `r round(res["b[4]","Mean"],3)`.  This is significantly closer to 0 than that
of genotype DWF plants, implying that the xf concentration falls of
 less (as a function of distance) in genotype WT plants, as
...
For DSF sounce plants, $\beta_5$ and $\beta_7$ determine the linear
...
interested in $\beta_7$ corefficient, which is approximately equal to 0.00034.  This means that for DSF source
plants, the vector xf concentration is approximately 0.00034 of the source plant xf concentration (at
the appropriate distance).

For WT sounce plants, $\beta_6$ and $\beta_8$ determine the linear
relationship for how xf bacteria is transmitted to vectors, as
$\lambda_{vector} = \beta_6 + \beta_8 \cdot \lambda_{source\ plant}$.  We are
interested in $\beta_8$ corefficient, which is approximately equal to 0.00085.  This means that for WT source
plants, the vector xf concentration is approximately 0.00085 of the source plant xf concentration (at
the appropriate distance).  Note this is a much higher transmission
rate (from source plants to vectors) for WT plants, than for DSF plants.
...
`r round(1/(1+exp(-res["b[9]","CI Upper"])), 2)`).
...
`r round(1/(1+exp(-res["b[10]","CI Upper"])), 2)`).
...
Both of these predictions use the value distance = 0.
